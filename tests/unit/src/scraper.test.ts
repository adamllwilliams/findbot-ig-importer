...
describe('scrapePost', () => {
  ...
  it('should return imageBase64 and caption on happy path', async () => {
    ...
    jest.spyOn(global, 'fetch')
      .mockResolvedValueOnce({
        ok: true,
        json: async () => [{ displayUrl: 'https://cdn.instagram.com/image.jpg', caption: 'Test event caption' }]
    ...
    // Act
    const result = await scrapePost('https://www.instagram.com/p/ABC123/');
  ...
  it('should throw error if apify request fails', async () => {
    ...
    // Act & Assert
    await expect(scrapePost('https://www.instagram.com/p/ABC123/')).rejects.toThrow('Apify request failed with status 403');
  });
  ...
  it('should throw error if apify items are empty', async () => {
    ...
    // Act & Assert
    await expect(scrapePost('https://www.instagram.com/p/ABC123/')).rejects.toThrow('No results returned from Apify');
  });
  ...
  it('should throw error if apify response is missing required fields', async () => {
    ...
    jest.spyOn(global, 'fetch')
      .mockResolvedValueOnce({
        ok: true,
        json: async () => [{ displayUrl: 'https://cdn.instagram.com/image.jpg' }]
      } as unknown as Response);

    // Act & Assert
    await expect(scrapePost('https://www.instagram.com/p/ABC123/')).rejects.toThrow();
  });
  ...
  it('should use the first item if multiple items are returned from apify', async () => {
    ...
    jest.spyOn(global, 'fetch')
      .mockResolvedValueOnce({
        ok: true,
        json: async () => [
          { displayUrl: 'https://cdn.instagram.com/image1.jpg', caption: 'First caption' },
          { displayUrl: 'https://cdn.instagram.com/image2.jpg', caption: 'Second caption' }
        ]
      } as unknown as Response)
      .mockResolvedValueOnce({
        ok: true,
  ...
  it('should proceed with image if caption is empty', async () => {
    ...
    jest.spyOn(global, 'fetch')
      .mockResolvedValueOnce({
        ok: true,
        json: async () => [{ displayUrl: 'https://cdn.instagram.com/image.jpg', caption: '' }]
      } as unknown as Response)
      .mockResolvedValueOnce({
        ok: true,
        arrayBuffer: async () => Buffer.from('fake-image-data')
  ...
  it('should throw an error with empty imageBase64 and caption', async () => {
    ...
    // Act & Assert
    await expect(scrapePost('https://www.instagram.com/p/ABC123/')).rejects.toThrow('No image or caption available for event extraction');
    expect(warnSpy).toHaveBeenCalledWith('Failed to fetch image, proceeding with caption only');
  });
});