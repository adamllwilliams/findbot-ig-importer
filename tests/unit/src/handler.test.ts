...
describe('importPost', () => {
  it('should return 400 if url is missing', async () => {
    ...
    expect(JSON.parse(result.body as string).error).toBe('A valid Instagram URL is required');
  });
  ...
  it('should return 200 with event data on happy path', async () => {
    ...
    // Act
    const result = await importPost(mockEvent('https://www.instagram.com/p/ABC123/')) as { statusCode: number; body: string };
